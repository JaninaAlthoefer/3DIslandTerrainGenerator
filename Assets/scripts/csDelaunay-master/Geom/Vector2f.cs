...
public struct Vector2f {
    ...
    //implementation to convert the custom vector of the cs-delaunay-master to be used on Dictionaries to remove duplicates
    //when generating the voronoi
    public static explicit operator Vector2Int(Vector2f v)
    {
        Vector2Int res = new Vector2Int((int) v.x, (int) v.y);

        return res;
    }

    //new implementation to divide vectors by value
    //used to get middle of two corners to find a coord for edges 
    //--> as a value to reference an edge by
    public static Vector2f operator /(Vector2f a, float d)
    {
        Vector2f res = new Vector2f(a.x / d, a.y / d);
